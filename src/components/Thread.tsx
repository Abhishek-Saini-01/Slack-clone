...
import { useGenerateUploadUrl } from '@/features/upload/api/useCreateWorkspace'
...
const Thread = ({
    ...
    const isLoadingMore = status === "LoadingFirstPage";

    const groupedMessages = results?.reduce((gruops, message) => {
        const date = new Date(message._creationTime);
        const dateKey = format(date, "yyyy-MM-dd");
        if (!gruops[dateKey]) {
            gruops[dateKey] = []
        }
        gruops[dateKey].unshift(message);
        return gruops;
        ...
        <div className='h-full flex flex-col'>
            ...
            <div className="flex-1 flex flex-col-reverse pb-4 overflow-y-auto messages-scrollbar">
                {Object.entries(groupedMessages || {}).map(([dateKey, messages]) => (
                    <div key={dateKey}>
                        <div className="text-center select-none my-2 relative">
